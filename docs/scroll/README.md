...
| Sidebar nav (menu + sous-menus) | `src/components/dashboard/DashboardV2.components.jsx` (`AppSidebar`) | `sidebar-nav-scroll` |
...
- **2026-08-08** — `AppSidebar` : scroll menu/sous-menus via `ModalScrollColumn` + `sidebar-nav-scroll` (retire `transform`/`scrollBehavior:smooth` qui mangeaient le trackpad).
