...
/** Repli localStorage pour le refresh (cookies seuls → x-refresh-token absent → forceLogout srv-*). */
const LEGACY_REFRESH_LOCALSTORAGE_KEY = 'sojori_refresh_token';
...
/**
 * Refresh token : cookie `sojori_refresh_token`, sinon repli localStorage (aligné access token).
 */
export const getRefreshToken = (): string | undefined => {
  const fromCookie = getCookie(AUTH_CONFIG.REFRESH_TOKEN_KEY);
  if (fromCookie) return fromCookie;
  if (typeof window === 'undefined') return undefined;
  try {
    const legacy = window.localStorage.getItem(LEGACY_REFRESH_LOCALSTORAGE_KEY)?.trim();
    return legacy || undefined;
  } catch {
    return undefined;
  }
...
export const setTokens = (token: string, refreshToken: string): void => {
  ...
  if (!access) return;
  setCookie(AUTH_CONFIG.TOKEN_KEY, access);
  const refresh = refreshToken?.trim();
  if (refresh) {
    setCookie(AUTH_CONFIG.REFRESH_TOKEN_KEY, refresh);
  }
  if (typeof window !== 'undefined') {
    try {
      window.localStorage.setItem(LEGACY_JWT_LOCALSTORAGE_KEY, access);
      if (refresh) {
        window.localStorage.setItem(LEGACY_REFRESH_LOCALSTORAGE_KEY, refresh);
      }
    } catch {
...
export const clearTokens = (): void => {
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      window.localStorage.removeItem(LEGACY_REFRESH_LOCALSTORAGE_KEY);
    } catch {
      /* ignore */
    }
  }
