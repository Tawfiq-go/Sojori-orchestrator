...
const MailTemplates = () => {
  ...
  const renderContent = () => {
    ...
    const showCreateButton = !showForm && activeTab === 'message';
    switch (activeTab) {
      case 'message':
        return <div className="mail-templates-container">
            {/* Sticky Header Section */}
            <div className="mail-templates-header">
              <Typography variant="h4" component="h1" gutterBottom sx={{
              mb: 0,
              fontWeight: 600,
              color: '#1a202c'
            }}>
                {t('Mail Template Management')}
              </Typography>
            </div>

            {/* Filter Bar - Also part of sticky header */}
            <Box className="mail-templates-filters" sx={{
            display: 'flex',
            alignItems: 'center',
            gap: 2,
            ...
            zIndex: 999
              ...
              flex: 1
                ...
                {/* Search */}
                <TextField placeholder={t('Search templates...')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} size="small" sx={{
                minWidth: '300px'
              }} InputProps={{
                ...
                {isAdmin && <>
                  ...
                  setFilterMode('admin');
                  setSelectedOwner('');
                  handleFilterChange('filterMode', 'admin');
                  ...
                  setSelectedOwner('');
                  handleFilterChange('filterMode', 'owners');
                  ...
                  handleFilterChange('selectedOwner', newValue ? newValue._id : '');
              ...
              gap: 2
                ...
                {/* Create Button */}
                {canCreate && showCreateButton && <Button variant="contained" startIcon={<AddIcon />} onClick={handleCreate} className="mail-templates-create-btn" sx={{
                backgroundColor: SOJORI_COLORS.primary,
                '&:hover': {
                  backgroundColor: SOJORI_COLORS.primaryDark
                }
            ...
            {isLoading ? <div className="flex items-center justify-center h-64">
              ...
              color: SOJORI_COLORS.primary
                ...
                <TableContainer component={Paper} className="mail-templates-table-container">
                  <Table stickyHeader className="mail-templates-table">
                    <TableHead>
                      <TableRow>
                        <TableCell>{t('Name')}</TableCell>
                        <TableCell align="center">{t('Type')}</TableCell>
                        <TableCell align="center">{t('Status')}</TableCell>
                        <TableCell align="center">{t('Messages')}</TableCell>
                        <TableCell align="center">{t('Canal')}</TableCell>
                    ...
                    <TableBody>
                      ...
                      py: 8
                        ...
                        </TableRow> : paginatedTemplates.map(template => {
                    const owner = owners.find(o => o._id === template.ownerId);
                    return <TableRow key={template._id} hover onClick={() => handleRowClick(template)}>
                            {/* Name */}
                            <TableCell>
                              <div className="mail-templates-name">
                                <EmailIcon className="mail-templates-name-icon" />
                                <span className="mail-templates-name-text">
                                  {t(template.messageName)}
                                </span>
                              </div>
                            </TableCell>
                            ...
                            <TableCell align="center">
                              <Chip label={template.enabled ? t('Active') : t('Inactive')} size="small" color={template.enabled ? 'success' : 'default'} icon={template.enabled ? <CheckIcon /> : <CancelIcon />} className="mail-templates-status-chip" onClick={e => handleStatusClick(e, template)} sx={{
                          cursor: 'pointer',
                          '&:hover': {
                            opacity: 0.8
                          }
                            ...
                            {/* Messages (icons) */}
                            <TableCell align="center">
                              <div className="mail-templates-language-icons">
                                {template.content && <span>🇫🇷</span>}
                                {template.contentEng && <span>🇬🇧</span>}
                              </div>
                            </TableCell>

                            {/* Canal & Priorité - Clickable */}
                            <TableCell align="center" onClick={e => handleChannelClick(e, template)} sx={{
                        cursor: 'pointer',
                        '&:hover': {
                          backgroundColor: '#FFF3E0'
                              ...
                              {(() => {
                          const hasOTA = template.messageEnabled && template.content;
                          ...
                          if (hasOTA && hasWhatsApp) {
                            ...
                            if (priority === 'OTA_PRIORITY') {
                              ...
                              label = 'OTA uniquement';
                              color = 'default';
                              icon = '📧';
                            } else {
                              label = 'Orchestrateur décide';
                              color = 'info';
                              icon = '🔄';
                            }
                          } else if (hasOTA && !hasWhatsApp) {
                            label = 'OTA uniquement';
                            color = 'default';
                            icon = '📧';
                          } else if (!hasOTA && hasWhatsApp) {
                          ...
                          return <Box sx={{
                            ...
                            gap: 0.5
                                    ...
                                    <Chip label={`${icon} ${label}`} size="small" color={color} sx={{
                              fontWeight: 500,
                              minWidth: '140px'
                            }} />
                          ...
                          const descriptions = {
                            'CONFIRMATION_RESERVATION_AVEC_LIEN': 'Sent immediately after booking confirmation. Includes WhatsApp link for guest communication.',
                            'RAPPEL_X_JOURS_AVANT_ARRIVEE': 'Reminder sent X days before arrival. Prepares guest for upcoming stay.',
                            'RAPPEL_ENREGISTREMENT_MANQUANT': 'Reminds guest to complete registration if missing information.',
                            'RAPPEL_COMPLET_CLIENT': 'Prompts guest to finalize check-in process with all required details.',
                            'RAPPEL_HORAIRE_MANQUANT': 'Requests arrival time confirmation from guest.',
                            'RAPPEL_DEPART': 'Sent day before arrival. Final preparation and arrival instructions.',
                            'RAPPEL_SERVICES_PENDANT_SEJOUR': 'During stay communication. Access to concierge services.',
                            'RAPPEL_ACTIONS_MANQUANTES': 'Pre-departure reminder. Checkout instructions and requirements.',
                            'SOLLICITATION_NOTE_ABSENTE': 'Post-stay review request sent after guest checkout.'
                          };
                          return descriptions[template.messageName] || 'Custom template';
                        })()}>
                            ...
                            color: '#333'
                              ...
                              gap: 1
                                    ...
                                    {/* Badge d'origine */}
                                    {template.configurationSource === 'AI' && <Chip label="🤖 AI" size="small" sx={{
                                fontSize: '0.7rem',
                                height: 20,
                                ...
                                fontWeight: 600
                                    ...
                                    {template.configurationSource === 'FROM_TEMPLATE' && <Chip label="📋 Template" size="small" sx={{
                                fontSize: '0.7rem',
                                height: 20,
                                ...
                                fontWeight: 600
                                    ...
                                    {template.configurationSource === 'MANUAL' && <Chip label="✏️ Manuel" size="small" sx={{
                                fontSize: '0.7rem',
                                height: 20,
                                backgroundColor: '#FFF3E0',
                                color: '#E65100',
                                fontWeight: 600
                                    ...
                                    {template.configurationSource === 'SEED' && <Chip label="🌱 Admin" size="small" sx={{
                                fontSize: '0.7rem',
                                height: 20,
                                ...
                                fontWeight: 600
                                  ...
                                  const shortDesc = {
                                    'CONFIRMATION_RESERVATION_AVEC_LIEN': 'booking confirmation + WhatsApp',
                                    'RAPPEL_X_JOURS_AVANT_ARRIVEE': 'pre-arrival reminder',
                                    'RAPPEL_ENREGISTREMENT_MANQUANT': 'registration reminder',
                                    'RAPPEL_COMPLET_CLIENT': 'complete check-in prompt',
                                    'RAPPEL_HORAIRE_MANQUANT': 'arrival time request',
                                    'RAPPEL_DEPART': 'final arrival instructions',
                                    'RAPPEL_SERVICES_PENDANT_SEJOUR': 'in-stay services access',
                                    'RAPPEL_ACTIONS_MANQUANTES': 'pre-departure checkout',
                                    'SOLLICITATION_NOTE_ABSENTE': 'post-stay review request'
                                  };
                                  return shortDesc[template.messageName] || '-';
                                })()}
                                    </span>
                                  </Box>
                                </Typography>
                            ...
                            <TableCell align="center">
                            ...
                            }} className="mail-templates-action-btn">
                                    <VisibilityIcon />
                                  </IconButton>
                                </Tooltip>
                                {canUpdate && <Tooltip title={t('Edit Template')}>
                              ...
                              color: SOJORI_COLORS.primary
                                ...
                                {canCreate && filterMode === 'orchestrator' && <Tooltip title={t('Initialiser ce template')}>
                                    <IconButton size="small" onClick={e => handleInitializeSingleTemplate(template.messageName, e)} sx={{
                              color: '#4CAF50',
                              '&:hover': {
                ...
                <Box className="mail-templates-pagination" component={Paper} elevation={1}>
                ...
                {/* Modal */}
                <TemplateDetailModal open={modalOpen} template={selectedTemplate} owner={selectedOwner} onClose={() => setModalOpen(false)} onEdit={handleEdit} onDelete={handleDeleteConfirmFromModal} canUpdate={canUpdate} canDelete={canDelete} />
              </>}
          </div>;
      case 'description':
        if (!SHOW_MAIL_TEMPLATE_DESCRIPTION_TAB) {
