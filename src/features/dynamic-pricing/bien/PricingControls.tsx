...
import { Box, Stack, Typography, Slider, Button, IconButton } from '@mui/material';
...
export interface PricingEvent {
  ...
  emoji: string;
  name: string;
  dateRange: string;        // "14-18 mars 2027"
  fixedPrice: number;       // MAD
  minNights: number;
}
...
export interface PricingControlsProps {
  floor: number;
  ceiling: number;
  floorRange: [number, number];     // [min, max] slider bounds
...
export default function PricingControls(props: PricingControlsProps) {
  const {
    ...
    mode, events, suggestions, onFloorChange, onCeilingChange, onApplyRecoBounds,
  ...
  return (
    <Box sx={{
      ...
      <CtrlBlock title="💰 Bornes de prix">
        ...
        <Box sx={{ mb: 2.25 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ mb: 0.75 }}>
            <Typography sx={lblSx}>Prix plancher</Typography>
        ...
        <Box sx={{ mb: 2.25 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ mb: 0.75 }}>
            <Typography sx={lblSx}>Prix plafond</Typography>
      ...
      {/* Bloc B : Mode */}
      <CtrlBlock title="🎯 Mode d'agressivité">
        <ModeCard
          on={mode === 'prudent'} onClick={() => onModeChange('prudent')}
          name="Prudent"
          desc={<>Cible <b>60%</b> occupation · prix moyen marché</>}
          quote="Je veux remplir mon calendrier"
        />
        <ModeCard
          on={mode === 'equilibre'} onClick={() => onModeChange('equilibre')}
          name="Équilibré" recommended
          desc={<>Cible <b>45%</b> occupation · prix légèrement au-dessus</>}
          quote="L'optimum revenu × occupation"
          extra={estimatedRevenue !== undefined && (
        ...
        />
        <ModeCard
          on={mode === 'agressif'} onClick={() => onModeChange('agressif')}
          name="Agressif"
          desc={<>Cible <b>30%</b> occupation · prix premium</>}
          quote="Je vise le segment premium"
        />
      </CtrlBlock>

      {/* Bloc C : Events */}
      <CtrlBlock title="📅 Événements spéciaux">
        ...
        {events.map(ev => (
          <Stack key={ev.id} direction="row" alignItems="center" gap={1.25} sx={{
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              <Typography sx={{
                ...
                {ev.dateRange} · <b style={{ color: T.goldDeep }}>{ev.fixedPrice} MAD/nuit</b> · min {ev.minNights} nuits
              </Typography>
            </Box>
        ...
        {suggestions.length > 0 && (
          <Box sx={{
            ...
            {suggestions.map(s => (
              <Stack key={s.id} direction="row" alignItems="center" gap={1.125}
                onClick={() => onAcceptSuggestion(s.id)} sx={{
...
function ModeCard({ on, recommended, name, desc, quote, extra, onClick }: {
  on: boolean; recommended?: boolean; name: string;
  desc: React.ReactNode; quote: string; extra?: React.ReactNode; onClick: () => void;
}) {
  return (
    <Box onClick={onClick} sx={{
      p: 1.75, mb: 1.25, borderRadius: 1.5, cursor: 'pointer', position: 'relative',
      bgcolor: T.bg1,
      ...
      <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 0.625, pr: 4 }}>
