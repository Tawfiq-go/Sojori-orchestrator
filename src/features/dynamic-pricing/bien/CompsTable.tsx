  ...
  return (
    <Box sx={{
      ...
      <Stack direction="row" sx={{ alignItems: 'center', gap: 1.5, 
        ...
        <Typography sx={{ fontSize: 10.5, color: T.text3, ml: 1, flex: 1 }}>
          Lecture seule · snapshot ⟳ · tri sur les colonnes · carte §06
        </Typography>
      </Stack>
          ...
          <Box component="tbody">
            {sortedRows.map(row => (
              <Box component="tr" key={row.id} sx={{
                ...
                <Td>
                  {row.isSelf ? (
                    <Box component="b" sx={{ color: T.goldDeep, fontWeight: 700 }}>
                      {row.name} (vous)
                    </Box>
                  ) : (() => {
