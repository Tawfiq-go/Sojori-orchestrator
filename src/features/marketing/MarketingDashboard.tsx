...
/**
 * Périmètre du premier établissement instrumenté.
 *
 * En dur le temps que le rattachement campagne→établissement couvre plusieurs
 * clients : la table `campaign_listing_map` n'a pour l'instant que Nommos, et
...
function Kpi({
  ...
  return (
    <Box sx={{ ...cardSx, p: 2 }}>
      ...
      {sub && (
        <Typography sx={{ fontSize: 12, color: T.mut, mt: 0.25 }}>
          {sub}
        </Typography>
      )}
    </Box>
...
function Panel({
  ...
  return (
    <Box sx={{ ...cardSx, mb: 2.5 }}>
      <Typography sx={{ fontSize: 15, fontWeight: 650, color: T.ink }}>
        {title}
      </Typography>
      {desc && (
        <Typography sx={{ fontSize: 12.5, color: T.mut, mt: 0.25, mb: 1.5 }}>
          {desc}
        </Typography>
      )}
      {children}
...
export default function MarketingDashboard() {
  const [connections, setConnections] = useState<MarketingConnection[] | null>(
    null,
  );
    ...
    <Box sx={{ bgcolor: T.bg, minHeight: "100%", p: { xs: 2, md: 3 } }}>
      <Box sx={{ mb: 2.5 }}>
        ...
        <Typography sx={{ fontSize: 13.5, color: T.mut, mt: 0.5 }}>
          Dépense publicitaire face aux réservations réelles · {range.from} →{" "}
          {range.to}
        </Typography>
      </Box>
      ...
      {error && (
        <Alert severity="info" sx={{ mb: 2.5 }}>
          Les données ne sont pas encore disponibles : le service de collecte
          n'est pas branché derrière l'API d'administration. ({error})
        </Alert>
      )}
      ...
      {noConnection && !error && (
        <Box sx={{ ...cardSx, textAlign: "center", py: 5 }}>
          <Typography
            sx={{ fontSize: 16, fontWeight: 650, color: T.ink, mb: 1 }}
          >
          ...
          >
            Un compte devient suivi une fois que son propriétaire a accordé un
            accès en lecture et que la connexion a été enregistrée. Un partage
            seul ne déclenche aucune collecte — c'est volontaire : un accès
            accordé par erreur ne doit rien lire.
          </Typography>
        </Box>
      )}
      ...
      {overview && (
        <>
          ...
          >
            <Kpi
              label="Dépense"
              value={`${fmt(overview.totals.spendMad)} MAD`}
            />
          ...
          {(overview.totals.directReservations != null ||
            ...
            >
              <Stack direction="row" spacing={2}>
                ...
                >
                  <Typography sx={{ ...kickerSx, color: T.direct }}>
                    Direct
                  </Typography>
                  <Typography
                    sx={{
                ...
                >
                  <Typography sx={{ ...kickerSx, color: T.ota }}>
            ...
            >
              ...
              <Box sx={{ mt: 2, overflowX: "auto" }}>
                ...
                >
                  <Box component="thead">
                    <Box component="tr">
                      {["Pays", "Dépense", "Clics", "CTR", "Réservations"].map(
                        (h, i) => (
                          <Box
                            key={h}
                            component="th"
                  ...
                  <Box component="tbody">
                    {overview.byCountry.map((r) => (
                      <Box component="tr" key={r.country}>
                        ...
                        >
                          {FLAGS[r.country] ?? ""}{" "}
                          {COUNTRY_LABEL[r.country] ?? r.country}
                        </Box>
                        {[
                          `${fmt(r.spendMad)} MAD`,
                          fmt(r.clicks),
          ...
          {scores && scores.campaigns.length > 0 && (
            <CampaignScores data={scores} />
          )}

          <Panel
            title="Campagnes — diffusion brute"
            desc="Ce que Meta a facturé, sans rapprochement. Le tableau ci-dessus dit ce que chaque campagne a produit."
          >
