...
const WorkerFilters = ({
  ...
  const handleAddClick = () => {
    ...
    navigate(
      workerTypeOwner ? `/admin/User/create-owner-user` : `/admin/User/create-user`
    );
  };
          ...
          <ListingGlobalFilter
            searchFilter={
              <div className="flex items-center">
                ...
                <Tooltip
                  title={<span style={{ whiteSpace: 'pre-line', fontSize: 15 }}>{t('Search by name, email, phone')}</span>}
                  arrow
                  placement="bottom"
                >
                  <IconButton
            ...
            showAddButton={Boolean(canCreate)}
            onAddClick={handleAddClick}
            canCreate={canCreate}
            addButtonLabel={t('Invite')}
          />
        }
        paginationContent={
          totalItems > 0 ? (
