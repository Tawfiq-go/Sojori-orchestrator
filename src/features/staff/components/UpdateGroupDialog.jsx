// pages/admin/groups/UpdateGroupSidebar.jsx
...
export default function UpdateGroupSidebar({ open, onClose, group, onSubmit }) {
  const { t } = useTranslation('common');
  const featureRows = useMemo(() => buildFeatureRows(routes, t, 'Owner'), [t]);

  // lock page scroll while sidebar is open (same behavior as AddSidebarTask)
  useEffect(() => {
    if (open) {
  ...
  const initial = {
    ...
    isAdminLevel:
      Array.isArray(group?.featureGrants) &&
      group.featureGrants.some(
        (g) => g.feature === '*' || (g.actions || []).includes('*'),
      ),
    featureGrants: Array.isArray(group?.featureGrants)
      ? group.featureGrants
      : [],
  };
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: 13, color: 'rgba(255,255,255,0.9)' }}>
                {group?.name || t('Update group permissions')}
              </Typography>
            </Box>
          </Box>
          <IconButton
