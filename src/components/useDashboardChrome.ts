...
/** Overrides locaux (historique) — le reste vient de navRoutes.ts (aligné navConfig). */
...
export function useDashboardChrome() {
  ...
  const getActivePathFromUrl = () => {
    ...
    if (path.startsWith('/finances/landlords')) {
      return 'finances/landlords';
    }
    ...
    if (path === '/menage/semaine' || path.startsWith('/menage/semaine/')) {
