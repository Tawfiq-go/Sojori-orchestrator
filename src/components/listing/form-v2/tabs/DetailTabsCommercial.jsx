...
import { T, sxInput, Field, Card, SectionH, ToggleRow, Counter, ChipsRow, NumberInput, MoneyInput, SelectField, AiBanner } from './_shared';
    ...
    <Box>
      ...
      <Card title="💰 Prix de base">
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(3, 1fr)' }, gap: 1.5 }}>
          <Field label="Prix par nuit" required ai>
          ...
          <Field label="Prix week-end" hint="Override Ven/Sam">
    ...
    <Box>
      <Card title="🚪 Check-in / Check-out" meta="checkInTime · checkOutTime">
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1.5 }}>
          <Field label="Plage check-in" hint="checkInTimeStart → checkInTimeEnd">
          ...
          <Field label="Heure check-out" required>
      ...
      <Card title="📆 Règles de réservation">
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5, mb: 1.5 }}>
          <Field label="Nuits min"><Counter value={values.minNights ?? 2} onChange={v => upd('minNights', v)} /></Field>
          <Field label="Nuits max"><Counter value={values.maxNights ?? 30} onChange={v => upd('maxNights', v)} /></Field>
          <Field label="Préavis arrivée (h)"><NumberInput value={values.advanceNotice ?? 24} suffix="h" onChange={v => upd('advanceNotice', v)} /></Field>
        </Box>
        <ToggleRow title="Instant booking" desc="Réservation immédiate sans approbation manuelle (recommandé Airbnb Plus)" checked={!!values.instantBooking} onChange={v => upd('instantBooking', v)} />
    ...
    <Box>
      <Card title="🧹 Frais de ménage">
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
          <Field label="Frais ménage (forfaitaire)" hint="Inclus dans le prix total"><MoneyInput value={values.cleaningFee ?? 80} onChange={v => upd('cleaningFee', v)} currency="€" /></Field>
      ...
      <Card title="💳 Caution / Dépôt de garantie">
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mt: 1 }}>
          <Field label="Montant caution"><MoneyInput value={values.securityDeposit ?? 500} onChange={v => upd('securityDeposit', v)} currency="€" /></Field>
