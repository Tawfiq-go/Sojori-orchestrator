...
import { SojoriLogsSourceNav } from './SojoriLogsSourceNav';
...
export function SojoriLogsHubContent({
  hideSourceNav = false,
  unifiedContext = false,
}: {
  hideSourceNav?: boolean;
  unifiedContext?: boolean;
}) {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      if (!body?.success) {
        setError(body?.error || 'Erreur chargement des logs AirROI');
        setListData(null);
        return;
      }
  ...
  }, [page, endpointTab, ownerFilter, dateQuery]);
  ...
  return (
    <div className="sojori-logs-hub space-y-2">
      <SojoriLogsToolbar
        onRefresh={loadList}
        loading={loading}
        title={unifiedContext ? 'Unifié · AirROI' : undefined}
        subtitle={
          unifiedContext
            ? 'Appels API marché & listings — source AirROI'
            : undefined
        }
      />

      <div className={`${hideSourceNav ? '' : 'sojori-logs-layout flex flex-col lg:flex-row gap-3 items-start'}`}>
        {!hideSourceNav ? <SojoriLogsSourceNav /> : null}

        <div className={`${hideSourceNav ? 'w-full' : 'flex-1 min-w-0'} space-y-2 w-full`}>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  ...
                  {!loading && !items.length ? (
                    <tr>
                      <td colSpan={9} className="px-2 py-8 text-center text-slate-400">
                        Aucun appel AirROI pour ce filtre.
                      </td>
                    </tr>
                  ) : null}
                  {items.map((row) => {
                    ...
                    const recapText = resolveAirroiRowRecap(row);
                    return (
                      <React.Fragment key={id}>
                        <tr className="border-t border-slate-100 hover:bg-orange-50/30 cursor-pointer">
                          ...
                          <td className="px-2 py-1.5 text-[11px] text-slate-700 whitespace-nowrap">
                            {row.createdAt
                              ? formatCasablancaDate(String(row.createdAt), 'dd/MM HH:mm:ss')
                              : '—'}
                          </td>
                          <td className="px-2 py-1.5">
                            <div className="font-medium text-slate-800 text-[11px]">
                              {String(row.endpointLabel || row.endpoint || '')}
                            </div>
                            {row.endpoint ? (
                          ...
                          <td
                            className="px-2 py-1.5 text-[11px] sojori-logs-recap-cell"
                            title={recapText}
                          >
                            <span className={row.success ? 'text-slate-700' : 'text-red-700'}>
                              {recapText}
                            </span>
                          </td>
                          ...
                          <td
                            className="px-2 py-1.5 text-[10px] text-slate-500 truncate max-w-[100px]"
                            title={String(row.triggeredBy || '')}
                          >
