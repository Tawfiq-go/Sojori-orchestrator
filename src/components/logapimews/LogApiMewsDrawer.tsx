/** LogApiMews · Vue C — Détail d'un échange : JSON colorisé, copie, navigation ↑↓/Esc. */
import { useEffect, useMemo, useState } from 'react';
import {
  fetchLogApiMewsCallDetail,
  type LogApiMewsCallDetail,
  type LogApiMewsItem,
...
import { CatPill, DirBadge, StatusBadge } from './logApiMewsBits';
...
export interface LogApiMewsDrawerProps {
  callId: string | null;
  ids: string[];
  /** Item enrichi correspondant (noms listing/owner/résa) si présent dans la page courante du journal. */
  enriched?: LogApiMewsItem | null;
  onClose: () => void;
  onNavigate: (id: string) => void;
  onFilterCorrelation: (correlationId: string) => void;
}

function auditString(audit: Record<string, unknown>, key: string): string {
  const value = audit[key];
  return typeof value === 'string' ? value : '';
}

export function LogApiMewsDrawer({
  callId,
  ids,
  enriched,
  onClose,
  onNavigate,
  onFilterCorrelation,
}: LogApiMewsDrawerProps) {
  const [detail, setDetail] = useState<LogApiMewsCallDetail | null>(null);
  ...
  useEffect(() => {
    ...
    fetchLogApiMewsCallDetail(callId)
      .then((d) => {
  ...
  const audit = useMemo(() => detail?.auditContext ?? {}, [detail?.auditContext]);
  const contextIds = useMemo(() => {
    const out: string[] = [];
    for (const k of ['listingId', 'ownerId', 'accountId', 'sojoriReservationNumber']) {
      const v = auditString(audit, k);
      if (v) out.push(v);
    }
  ...
  const correlationId = auditString(audit, 'correlationId');
  const trigger = auditString(audit, 'trigger');
  const source = auditString(audit, 'modificationSource');
                ...
                <>
                  ...
                  <div className="dwr-meta">
                    ...
                    <div className="cell">
                      ...
                      <div className="val mono">{auditString(audit, 'route') || '—'}</div>
                    </div>
                    <div className="cell">
                  ...
                  <div className="xchange">
                    <XPanel
                      label="Requête"
                      side="req"
                      xml={detail.requestXml}
                      json={detail.requestJson ?? detail.requestPayload}
                    />
