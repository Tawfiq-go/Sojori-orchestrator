/** LogApiMews · Vue A — Synthèse : KPIs, santé par API (par catégorie), répartition par owner. */
import type {
  LogApiMewsActionStat,
  LogApiMewsCategory,
  LogApiMewsOwnerStat,
  LogApiMewsStatsResponse,
...
import { DirBadge, EmptyState, ErrorState, StatusBadge } from './logApiMewsBits';

function successRate(a: LogApiMewsActionStat): number {
...
function isDegraded(a: LogApiMewsActionStat): boolean {
...
export interface LogApiMewsSyntheseProps {
  stats: LogApiMewsStatsResponse | null;
  loading: boolean;
  error: boolean;
  onRetry: () => void;
...
export function LogApiMewsSynthese({
  stats,
  loading,
  error,
  onRetry,
  ...
  if (!stats || stats.kpis.total === 0) {
    return (
      <EmptyState
        ...
        detail="Aucun échange Mews Connector n’a été enregistré sur la fenêtre sélectionnée. Élargissez la période."
      />
    );
  }
  ...
  const byCategory = new Map<LogApiMewsCategory, LogApiMewsActionStat[]>();
  for (const a of byAction) {
    const category = CATEGORY_ORDER.includes(a.category as LogApiMewsCategory)
      ? (a.category as LogApiMewsCategory)
      : 'other';
    ...
    <div>
      <div className="kpis">
        ...
        <button
          ...
          title="Voir uniquement les échecs API Mews (pas les appels lents)"
        >
        ...
        <button
          ...
          title="Succès lents (>10s) ou rate limit Mews (429) — pas des échecs métier"
        >
          <div className="k">
      ...
      {byOwner.length > 0 && (
        <div className="catgroup">
          ...
          <table className="api-table">
            ...
            <tbody>
              {byOwner.map((o: LogApiMewsOwnerStat) => (
                <tr
                  key={o.ownerId}
