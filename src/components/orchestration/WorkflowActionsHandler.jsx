  ...
  const executeAction = useCallback(async (actionId, actionType) => {
    ...
    try {
      ...
      if (response.data?.success) {
        const message = response.data?.message || 'Action exécutée avec succès';
        ...
        const errorMsg = response.data?.error || 'Erreur lors de l\'exécution';
      ...
      const errorMsg = error.response?.data?.error || error.message || 'Erreur réseau';
