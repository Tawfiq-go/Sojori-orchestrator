...
/**
 * Messages OTA Page V2 - Nouveau design Claude
 * Route: /communications/messages-ota
 * Design: 2 colonnes (ThreadsList + ConversationThread)
 * Plateformes: Airbnb, Booking, VRBO
 */
export default function MessagesOTAPageV2() {
  ...
  const loadConversations = async () => {
    try {
      ...
      // Utiliser getOTAThreads si disponible, sinon getConversations avec filtre platform
      const response = await messagesService.getConversations({
        filter: activeFilter,
        search: searchTerm || undefined,
        hasReservation: true,
        platform: 'ota', // Filtre OTA
        limit: 50,
      });
  ...
  // Déterminer la couleur du channel selon la plateforme
  const getChannelColor = (platform?: string): string => {
    const platformLower = platform?.toLowerCase() || '';
    if (platformLower.includes('airbnb') || platformLower.includes('ab')) return '#FF5A5F';
    if (platformLower.includes('booking') || platformLower.includes('bk')) return '#003580';
    if (platformLower.includes('vrbo')) return '#003D79';
  ...
  const formattedThreads: Thread[] = conversations.map((conv) => ({
    ...
    phone: conv.phone,
    channel: 'ab', // OTA channel
    channelColor: getChannelColor(conv.platform),
  ...
  const quickTemplates: QuickTemplate[] = [
    { id: '1', label: '📋 Réponse standard', icon: '📋', text: 'Merci pour votre message. Nous revenons vers vous rapidement.' },
    { id: '2', label: '📅 Confirmer dates', icon: '📅', text: 'Nous confirmons votre réservation du [date] au [date].' },
    { id: '3', label: '💰 Détails paiement', icon: '💰', text: 'Concernant le paiement, voici les informations: ' },
  ];

  // Mock channel (pour ThreadsList qui en a besoin)
  const mockChannel = [{ id: 'ab', label: 'OTA', icon: '📨', color: '#FF5A5F', count: conversations.length }];
  ...
  const activeThread: Thread | null = activeConversation
    ? {
        ...
        phone: activeConversation.phone,
        channel: 'ab',
        channelColor: getChannelColor(activeConversation.platform),
        preview: '',
        time: '',
        unread: 0,
      ...
      {loading ? (
        ...
        <InboxLayout>
          ...
          >
            ...
            <Typography sx={{ fontSize: 13, color: t.text3 }}>
              Les messages OTA apparaîtront ici
            </Typography>
          </Box>
        </InboxLayout>
      ) : (
