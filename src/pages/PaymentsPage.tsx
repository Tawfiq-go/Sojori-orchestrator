...
export function PaymentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await paymentsService.getList({
        ...
        limit: 100,
        paymentStatus,
        reservationNumber: search.trim() || undefined,
        cardOnly,
      });
  ...
  }, [page, paymentStatus, search, cardOnly]);
  ...
  const stats = useMemo(() => {
    ...
    const withEvents = rows.filter((r) => (r.naps.events?.length || 0) > 0).length;
  ...
  return (
    <DashboardWrapper>
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: T.bg0, minHeight: '100vh' }}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
            ...
            <FormControl size="small" sx={{ minWidth: 180 }}>
              <Select
                ...
                onChange={(e) => setCardOnly(e.target.value === 'card')}
              >
                <MenuItem value="card">Carte / NAPS uniquement</MenuItem>
                <MenuItem value="all">Toutes réservations</MenuItem>
              </Select>
            </FormControl>
            <Button variant="contained" onClick={() => void load()} sx={{ bgcolor: T.primary }}>
              Filtrer
            </Button>
          </Stack>
        </Paper>
        ...
        <TableContainer component={Paper} elevation={0} sx={{ border: `1px solid ${T.border}` }}>
          {loading ? (
                    ...
                    return (
                      <Fragment key={row._id}>
                        <TableRow hover sx={{ '& > *': { borderBottom: open ? 0 : undefined } }}>
                          <TableCell>
                            <IconButton
                              ...
                              onClick={() => {
                                ...
                                setExpandedId(next);
                                if (!open && (!row.naps.events.length || !row.naps.payload)) {
                                  void loadDetail(row);
                                }
                              }}
                            >
                          ...
                          <TableCell>{row.naps.events?.length || 0}</TableCell>
                          <TableCell>
